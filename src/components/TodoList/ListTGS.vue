  ...
  <v-card width="600px">
    ...
    <v-data-table :headers="headers" :items="todos" :search="search">
        ...
        <template v-slot:[`item.checkbox`]="{item}">
            <v-checkbox v-model="hapusToDos" :value="item"/>
        </template>
    ...
    <v-card width="600px" class="mt-8" v-show="hapusToDos.length">
        <v-card-title>Delete Multiple</v-card-title>
         <ul v-for="(item,index) in hapusToDos" :key="index">
            <li>{{item.task}}</li>
        </ul> 
        <v-btn depressed color="error" class="ml-4" @click="hapusSemua">
            Hapus Semua
        </v-btn>
    </v-card>
...
export default {
  ...
  data(){
      return{
          ...
          details:false,
          deleted:false,
          headersHapus: {text: "Task", value: "task"},
          headers:[
            {
                text: "Task",
                align: "start",
        ...
        resetForm(){
         ...
         detailItem(item){
          ...
          edit(){   
           ...
           hapusSemua(){
               this.hapusToDos.forEach(element => {
                   if(this.detailsTodo.index==this.todos.indexOf(element)){
                        this.details=false;
                   }
